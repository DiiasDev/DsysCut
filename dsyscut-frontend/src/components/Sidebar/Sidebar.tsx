...
import { FiHome, FiDollarSign, FiCalendar, FiUsers, FiBarChart2, FiSettings, FiChevronLeft, FiChevronRight, FiX, FiChevronDown } from "react-icons/fi";
...
export default function Sidebar({
    ...
    // Para mobile, sidebar é modal/drawer de baixo para cima
    if (isMobile) {
        return (
            <>
                ...
                {!open && (
                    <button
                        className="fixed bottom-4 left-4 z-50 bg-[var(--color-accent)] text-white rounded-full p-2 shadow"
                        onClick={handleToggle}
                        aria-label="Abrir menu"
                    >
                        {React.createElement(FiChevronDown as any, { size: 24 })}
                    </button>
                )}
                {/* Overlay */}
                {open && (
                ...
                {/* Sidebar como drawer/modal de baixo para cima */}
                <aside
                    className={`fixed left-0 bottom-0 w-full ${open ? "h-[80vh]" : "h-0"} z-50 bg-[var(--color-bg-card)] shadow-lg transition-all duration-300 flex flex-col justify-between`}
                    style={{
                ...
                >
                    <div>
                        ...
                        <div className="flex items-center justify-between px-6 py-6 border-b border-[var(--color-bg)]">
                            <div
                            ...
                            {/* Remover nome do sistema no mobile */}
                            {/* <span className="font-semibold text-xl" style={{ color: "var(--color-primary)" }}>DsysCut</span> */}
                            <button
                                className="ml-auto text-xl opacity-70"
                                ...
                                {tabs.map(tab => {
                                    ...
                                    return (
                                        <li
                                            ...
                                            onClick={() => {
                                                handleTabClick(tab.key, tab.route);
                                                handleToggle();
                                            }}
                                        >
